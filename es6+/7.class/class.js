...
/*
    클래스의 필드 선언 : ES6+의 클래스가 가질 수 있는 제어자        // java - public protected default private
    - 퍼블릭 필드
    - 프라이빗 필드
    - 정적(스테틱) 퍼블릭 필드
    - 정적(스테틱) 프라이빗 필드
...
class Sample1 {
    ...
    constructor (publicField, privateField) {
        ...
        // this.staticPublicField = publicField;        // 잘못된 형태 사용X
        // this.#staticPrivateField = privateField;     // 잘못된 형태 사용X
    }
}

const sample1Instance = new Sample1('퍼블릭 필드', '프라이빗 필드');
console.log(sample1Instance);                           // Sample1 { publicField: '퍼블릭 필드' }
// console.log(sample1Instance.#privateField);          // 접근X
// console.log(sample1Instance.staticPublicField);      // undefinded
console.log(Sample1.staticPublicField);                 // 정적 퍼블릭필드


console.log('==================================================');

/*
    getter, setter :          // 캡슐화 관련 - private 필드에 접근할때 사용
...
console.log(sample2Instance.privateField);              // 프라이빗 필드    메서드지만 ()사용X 속성 사용할 때 처럼 사용
...
Sample3.staticMethod();             // 스태틱은 클래스로 직접호출해야 함
...
/*
    ...
    - 메서드 오버라이딩 가능                        // 오버라이드 : 부모 클래스에 정의된 것을 재정의
...
child2Instance.parentMethod();                  // 자식2 메서드 (오버라이딩 됨)
