...
/*
    ...
    - 자바스크립트는 배열 각 요소의 데이터 타입이 서로 달라도 됨
*/
const emptyArray = [];
...
for (const index in fruits) {           // 일반 for문(위에꺼)랑 결과가 같음, const index(index를 직접 가져옴), 변경작업시 in을 씀
    console.log(fruits[index]);
}
...
/*
    - for-in 순회 방식은 index 를 사용하여 한번 더 작업해야 하기 때문에 요소를 읽어들이는 순회에는 적합하지 않을 수 있음
    - forEach 메서드 혹은 for-of문을 사용
*/
for (const item of fruits) {        // item(값)을 직접 가져옴, for of 읽기 전용
    console.log(item);
}
...
/*
    slice(start, end) : 
    - start에 해당하는 인덱스 부터 end에 해당하는 인덱스 앞까지 복사햐여 반환
*/
...
/*
    ...
    - 배열의 요소를 제거하고 제거한 위치에 배열을 요소를 갈아끼운 배열을 반환
*/
