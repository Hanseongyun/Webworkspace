...
*/
var varVariable;
let letVariable;                // let도 일반변수 차후에는 전부 let으로 선언함 (var변수의 문제점)
const constVariable = 10;        // 자바스크립트에서는 상수 선언시 초기화 까지 해줘야됨
...
*/
// console.log(x);     // undefined 가 출력됨 
var x = 10;

if (true) {
    var x = 99;
}
// console.log(x);         // 99 출력 내부에서 선언했지만 외부에서 인식

/*
    var 키워드로 선언한 변수는 블록 레벨 스코프가 아닌 함수 레벨 스코프를 가짐
    - 함수 레벨 스코프 : 함수 내에서 선언된 변수는 함수 안에서 사용 가능, 함수 밖에선 사용 불가능 (ex- variable.js 내에서)
